...
def login_and_get_page(playwright_instance):
    ...
    # 1. Khởi tạo Chromium với danh sách args đã sửa dấu phẩy và bỏ --single-process
    browser = playwright_instance.chromium.launch(
        ...
        args=[
            ...
            "--no-first-run",
            "--no-zygote",
            "--disable-gpu",  # Đã thêm dấu phẩy ở đây
    ...
    if "account.base.vn" in page.url or page.query_selector("input[name='email']"):
        ...
        # Bấm Enter trực tiếp cho nhanh và ổn định hơn click nút
        ...
        # SỬA LỖI TIMEOUT: Chờ URL khớp với Regex hoặc String chuẩn
    ...
    try:
        ...
        print("-> Không có popup xuất hiện.")
    
    return browser, page


# THÊM KHỐI NÀY ĐỂ BẤM RUN LÀ CODE TỰ CHẠY
if __name__ == "__main__":
    with sync_playwright() as p:
        browser, page = login_and_get_page(p)
        print("Tiêu đề trang hiện tại:", page.title())
        # Giữ 5s để quan sát rồi đóng
        time.sleep(5)
        browser.close()
